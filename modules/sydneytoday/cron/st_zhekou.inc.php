...
$object = StZhekou::findById($id);
//$object = new StZhekou();
if (!$object) {
  die('Can not find StZhekou');
}
...
//  $crawler->read('http://www.sydneytoday.com/post.php?action=del&fid='.$fid.'&id='.$id);
  ...
  if (isset($matches[1])) {
    ...
    sendemailAdmin('Dingtie - SydneyToday折扣 error', 'For st_zhekou id:'.$object->getId().' == '.$error_msg);
    die('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />Error: ' . $error_msg);
  } else {
    // when we don't have '按优惠种类筛选' in the return result, we failed
    if (!mb_strpos($result, iconv('utf-8', 'gb2312', '按优惠种类筛选'), 0, 'gb2312')) {
      sendemailAdmin('Dingtie - SydneyToday折扣 error', "Error: failed to delete post");
...
$crawler->setReferer("http://www.sydneytoday.com/post.php?fid=194");
...
if (strpos($html, 'false') === false) {
  sendemailAdmin('Dingtie - SydneyToday折扣 error', 'For st_zhekou id:'.$object->getId().' == '.'Error: qacode failed');
...
if ($content == false) {
  sendemailAdmin('Dingtie - SydneyToday折扣 error', 'For st_zhekou id:'.$object->getId().' == '.'Error: content iconv failed');
...
$result = $crawler->post('http://www.sydneytoday.com/post.php?action=postnew', array(
    ...
    'postdb[sortid]' => $object->getSortid(),
    'postdb[my_expressurl]' => $object->getMyExpressurl(),
    'postdb[content]' => implode('', array_fill(0, rand(1,10), ' ')) .$content . implode('', array_fill(0, rand(1,20), ' ')),
...
if (isset($matches[1])) {
  $error_msg = iconv('gb2312', 'utf-8', $matches[1]);
  sendemailAdmin('Dingtie - SydneyToday折扣 error', 'For st_zhekou id:'.$object->getId().' == '.'Error: ' . $error_msg);
  die('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />Error: ' . $error_msg);
} else {
  $matches = array();
