...
//die($html);
  ...
  $data = "message=$message&posttime=$time&formhash=$formhash&usesig=1&subject=++" . ( (isset($secanswer) && isset($sechash)) ? "&sechash=$sechash&secanswer=$secanswer" : '');
...
// die("\n\n" . $data . "\n\n");
  ...
  if (strpos($html, 'succeedhandle_fastpost')) {
    ...
    $msg = 'Error when doing post reply. User is #' . $user->getId() . ' - ' . $user->getUsername() . ' == ' . (isset($secanswer) ? "<p>Secquestion: ".htmlentities($secquestion)."<br />Secanswer: $secanswer</p>" : "") . iconv('GBK', 'UTF-8', $html);
    $log = new Log('cron', Log::ERROR, $msg);
    $log->save();
